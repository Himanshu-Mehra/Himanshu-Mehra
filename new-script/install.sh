...
function install_jdk(){
	...
	#_java=$(ls /usr/lib/jvm | grep java)
	#mv /usr/lib/jvm/$_java /usr/lib/jvm/bkp_$_java
	if type -p java; then
		_java=java
	...
	if [[ "$_java" ]]; then
		...
		if [[ "$version" == "20.0.2" ]]; then
			...
			echo -e "[-] Found Current OpenJdk version $version, required version is OpenJdk 20.0.2"
			exit 0
		fi
	fi
}
